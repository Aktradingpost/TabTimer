...
async function openScheduledTab(lock, locks, settings, now) {
  try {
    ...
    if (lock.recurring && lockMinutes === 0) {
      // Never Lock recurring schedule — reschedule immediately right now.
      // autoRelockAt will never be set so we can't rely on the relock path.
      let nextUnlock = calculateNextUnlockTime(lock);
      let safetyCounter = 0;
      while (nextUnlock.getTime() <= now && safetyCounter < 1000) {
        lock.unlockTime = nextUnlock.toISOString();
        nextUnlock = calculateNextUnlockTime(lock);
        safetyCounter++;
      }
