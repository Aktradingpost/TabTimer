...
function loadSettingsIntoForm() {
  ...
  if (document.getElementById('settingGracePeriod')) {
    document.getElementById('settingGracePeriod').value = currentSettings.gracePeriodMinutes || 2;
  }
...
function showDuplicateTimeModal(lock) {
  return new Promise(async (resolve) => {
    ...
    customTimeInput.value = nextAvailableTime.toISOString().slice(0, 16);
...
function openBulkRescheduleModal() {
  ...
  document.getElementById('rescheduleStartTime').value = tomorrow.toISOString().slice(0, 16);
