...
chrome.runtime.onInstalled.addListener((details) => {
  console.log('TabTimer installed/updated:', details.reason);
  ...
  // Show welcome page on first install only (not on updates)
...
async function runScheduleHealthCheck() {
  ...
  // Check 3: Fix stuck recurring schedules with past times OR stuck in opened=true state
  for (const lock of locks) {
    if (lock.recurring) {
...
// Prevent concurrent runs of checkAndOpenLocks
let checkAndOpenLocksRunning = false;

async function checkAndOpenLocks() {
  if (checkAndOpenLocksRunning) {
    console.log('TabTimer: checkAndOpenLocks already running, skipping this tick');
    return;
  }
  checkAndOpenLocksRunning = true;
  try {
    await _checkAndOpenLocksImpl();
  } finally {
    checkAndOpenLocksRunning = false;
  }
}

async function _checkAndOpenLocksImpl() {
  ...
  for (const lock of locks) {
    ...
    if (now >= unlockTime && !lock.opened) {
      ...
      if (timeSinceScheduled > GRACE_PERIOD_MS) {
        ...
        if (lock.recurring) {
          ...
          } else if (rType === 'custom-days') {
            nextUnlock = new Date(now + (lock.customDays || 2) * 24 * 60 * 60 * 1000);
          } else {
            let candidate = new Date(lock.unlockTime);
            let safetyCounter = 0;
            while (candidate.getTime() <= now && safetyCounter < 365) {
              lock.unlockTime = candidate.toISOString();
              candidate = calculateNextUnlockTime(lock);
              safetyCounter++;
            }
    ...
    if (lock.opened && !lock.manuallyUnlocked && lock.autoRelockAt && lock.recurring) {
      ...
      if (now >= relockTime) {
        // Same fix as Never Lock path: base minutes/hours intervals on now,
        // use calendar math for daily/weekly/monthly so time-of-day is preserved
...
async function openScheduledTab(lock, locks, settings, now) {
  try {
    ...
    lock.reminderSent = false; // reset so next occurrence gets a reminder too
    
    const lockMinutes = lock.lockMinutes;
    if (lock.recurring && lockMinutes === 0) {
      // Never Lock recurring schedule — reschedule immediately.
      // IMPORTANT: Base the next occurrence on NOW, not on the old unlockTime.
      // If we advance from the old unlockTime, daily schedules jump to tomorrow
      // instead of firing again today, and minute/hour intervals land correctly
      // but daily/weekly land way in the future.
      const repeatType = lock.repeatType || 'daily';
      let nextUnlock;

      if (repeatType === 'minutes') {
        // Add interval to now so it fires X minutes from right now
        const mins = lock.minuteInterval || 10;
        nextUnlock = new Date(now + mins * 60 * 1000);
      } else if (repeatType === 'hourly') {
        // Add interval to now so it fires X hours from right now
        const hrs = lock.hourlyInterval || 1;
        nextUnlock = new Date(now + hrs * 60 * 60 * 1000);
      } else if (repeatType === 'custom-days') {
        // Add days to now
        const days = lock.customDays || 2;
        nextUnlock = new Date(now + days * 24 * 60 * 60 * 1000);
      } else {
        // For daily/weekly/monthly/yearly and others:
        // Keep the original TIME OF DAY from unlockTime, but advance the DATE
        // so it fires at the same time tomorrow (daily), next week, etc.
        // Use calculateNextUnlockTime which correctly handles calendar math,
        // but first make sure unlockTime is up-to-date by advancing past now.
        let candidate = new Date(lock.unlockTime);
        let safetyCounter = 0;
      ...
      lock.opened = false;
      lock.autoRelockAt = null;
      console.log(`TabTimer: Recurring (Never Lock) rescheduled "${lock.name || lock.url}" → ${nextUnlock}`);
    } else if (lockMinutes === 0) {
      // Non-recurring Never Lock — just mark as opened, nothing to reschedule
      lock.autoRelockAt = null;
    } else {
      // Has a lock — autoRelockAt path will handle rescheduling when lock expires
