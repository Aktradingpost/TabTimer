// service_worker.js - TabTimer Premium v2.0.0
...
chrome.runtime.onInstalled.addListener(() => {
  console.log('TabTimer Premium v2 installed');
...
// Repair stuck schedules - fix any recurring schedules that have past unlock times
async function repairStuckSchedules() {
  console.log('TabTimer: Checking for stuck schedules...');
  ...
  for (const lock of locks) {
    // Fix ALL recurring schedules that have past unlock times, regardless of opened status
    if (lock.recurring) {
      const unlockTime = new Date(lock.unlockTime).getTime();
      
      // If the unlock time is in the past, we need to fix it
      if (unlockTime < now) {
        ...
        console.log(`Repairing: ${lock.name || lock.url}`);
        console.log(`  Old time: ${lock.unlockTime}`);
        console.log(`  New time: ${nextUnlock.toISOString()}`);
        console.log(`  Iterations: ${safetyCounter}`);
  ...
  if (repaired > 0) {
    ...
    console.log(`TabTimer: Repaired ${repaired} stuck schedules`);
    ...
    chrome.notifications.create({
      ...
      title: 'TabTimer',
      message: `Repaired ${repaired} stuck schedules!`,
      priority: 2
    });
  } else {
    console.log('TabTimer: No stuck schedules found');
  }
...
async function checkAndOpenLocks() {
  ...
  // Options: 2 min, 30 min, 1 hr, 2 hr, 3 hr, 4 hr, 6 hr, 8 hr
  const gracePeriodMinutes = settings.gracePeriodMinutes || 2;
  ...
  for (const lock of locks) {
    ...
    if (now >= unlockTime && !lock.opened) {
      ...
      if (timeSinceScheduled > GRACE_PERIOD_MS) {
        ...
        if (lock.recurring) {
          // IMPORTANT: Keep calculating until we get a FUTURE time
      ...
      // Within grace period - open the tab
      try {
        ...
        // Track this tab to capture resolved URL after redirect
        trackTabForResolvedUrl(tab.id, lock.id);
        
        // Set auto-close - check per-schedule setting first, then global setting
        ...
        // Set auto-relock time based on lockMinutes (default 5)
        const lockMinutes = lock.lockMinutes || 5;
        lock.autoRelockAt = new Date(now + lockMinutes * 60 * 1000).toISOString();
    ...
    // This works for ALL schedules, not just recurring
    if (lock.manuallyUnlocked && lock.unlockedAt) {
      const lockMinutes = lock.lockMinutes || 5;
      ...
      if (now >= autoRelockTime) {
        // Auto re-lock: restore the schedule
        lock.manuallyUnlocked = false;
        lock.unlockedAt = null;
        
        // For recurring, schedule for next occurrence
        // For non-recurring, just re-lock (won't open again until time comes)
    ...
    if (lock.opened && !lock.manuallyUnlocked && lock.autoRelockAt && lock.recurring) {
      ...
      if (now >= relockTime) {
        // Calculate next unlock time based on repeatType
        // IMPORTANT: Keep calculating until we get a FUTURE time
        let nextUnlock = calculateNextUnlockTime(lock);
        let safetyCounter = 0;
        while (nextUnlock.getTime() <= now && safetyCounter < 365) {
          // Temporarily update lock.unlockTime so calculateNextUnlockTime works correctly
          lock.unlockTime = nextUnlock.toISOString();
          nextUnlock = calculateNextUnlockTime(lock);
          safetyCounter++;
        }
        ...
        // Check for date in name (YYYY-MM-DD format) for auto-delete
        const dateMatch = lock.name?.match(/(\d{4}-\d{2}-\d{2})/);
        if (dateMatch) {
          // Use configurable expiration time and timezone
          ...
          if (now > expDate.getTime()) {
            continue; // Don't reschedule, will be deleted
          }
        }
    ...
    // Auto-relock for NON-recurring schedules that have opened
    // After lockMinutes, mark them as ready to open again (but they won't since time has passed)
    if (lock.opened && !lock.manuallyUnlocked && lock.autoRelockAt && !lock.recurring) {
      const relockTime = new Date(lock.autoRelockAt).getTime();
      if (now >= relockTime) {
        // For non-recurring, just reset the opened flag
        // The schedule won't open again since the time has already passed
        lock.opened = false;
        lock.autoRelockAt = null;
        updated = true;
  ...
  if (settings.autoDeleteExpired) {
    ...
    const filtered = locks.filter(lock => {
      ...
      if (dateMatch) {
        ...
        try {
          // Create a date string that represents the expiration in the user's timezone
          const localExpDate = new Date(`${dateMatch[1]}T${expirationTime}:00`);
          // Get the offset for that timezone
...
async function createLock(lockData) {
  ...
  const newLock = {
    ...
    lockMinutes: lockData.lockMinutes || 5,
...
async function importFullBackup(data) {
  try {
    if (data.locks) {
      // Fix any schedules that have future times but are marked as opened
      const now = Date.now();
      const fixedLocks = data.locks.map(lock => {
        const unlockTime = new Date(lock.unlockTime).getTime();
        // If unlock time is in the future but marked as opened or manually unlocked, reset it
        if (unlockTime > now && (lock.opened || lock.manuallyUnlocked)) {
          console.log(`TabTimer: Fixing imported schedule "${lock.name}" - future time but marked as opened/unlocked`);
          return {
            ...lock,
            opened: false,
            manuallyUnlocked: false,
