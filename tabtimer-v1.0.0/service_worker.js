...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // Create auto-backup alarm (check every hour)
...
async function checkAndOpenLocks() {
  ...
  // Grace period: only open tabs if we're within 2 minutes of their scheduled time
  // This prevents a flood of tabs opening when computer wakes up or extension is re-enabled
  const GRACE_PERIOD_MS = 2 * 60 * 1000; // 2 minutes
  
  let updated = false;
  
  for (const lock of locks) {
    // Skip if already opened or manually unlocked
  ...
  try {
    switch (message.action) {
      ...
      case 'deleteLocks':
        ...
        break;
      case 'duplicateLock':
        sendResponse(await duplicateLock(message.id));
        break;
...
async function updateLock(updatedLock) {
  ...
  if (index !== -1) {
    locks[index] = { ...locks[index], ...updatedLock };
    await chrome.storage.local.set({ locks });
    return { success: true };
  }
...
async function importFullBackup(data) {
  try {
    if (data.locks) {
      await chrome.storage.local.set({ locks: data.locks });
    }
    if (data.settings) {
