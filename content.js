...
async function checkPageLock() {
  ...
  if (activeLock) {
    // If lockMinutes is 0 (Never), skip the overlay entirely — just let the page open normally
    if (activeLock.lockMinutes === 0) {
      return;
    }
...
async function showLockDialog(url, title) {
  ...
  const defaultLock = (settings.defaultLockMinutes !== undefined && settings.defaultLockMinutes !== null) ? settings.defaultLockMinutes : 0;
  ...
  // Get categories (includes custom premium ones)
  const allCategories = await chrome.runtime.sendMessage({ action: 'getCategories' });

  // Get license status for feature gating
  const licenseInfo = await chrome.runtime.sendMessage({ action: 'getLicense' });
  const hasPremium = licenseInfo && licenseInfo.isPremium;
  ...
  // Create overlay
  // Build notes HTML before template (avoids nested backtick issues in shadow DOM)
      ...
      <div class="tabtimer-body">
        ...
        <div class="tabtimer-field">
          ...
          <div class="tabtimer-btn-group" id="tabtimer-categories">
            ${(allCategories && allCategories.length > 0 ? allCategories : [
              {name:'Daily',icon:'📅'},{name:'Weekly',icon:'📆'},{name:'Monthly',icon:'🗓️'},{name:'Once',icon:'🎯'},{name:'Other',icon:'📌'}
            ]).map(cat => `<button type="button" class="tabtimer-btn-option" data-value="${cat.name}">${cat.icon || '📌'} ${cat.name}</button>`).join('')}
          </div>
        ...
        <div class="tabtimer-field">
          ...
          <div class="tabtimer-btn-group" id="tabtimer-repeat">
            ...
            <button type="button" class="tabtimer-btn-option" data-value="yearly">📅 Yearly</button>
            ${hasPremium ? `
          ...
          <div id="tabtimer-other-options" style="display: none; margin-top: 10px;">
            ...
            <!-- Every X Minutes -->
            <div id="tabtimer-minutes-group" style="display:none; margin-top:8px;">
              <label style="font-size:12px;">Repeat every <input type="number" id="tabtimer-minute-interval" min="1" max="59" value="10" style="width:55px;display:inline;padding:4px;border:1px solid #e2e8f0;border-radius:4px;"> minutes</label>
            </div>
            <!-- Every X Hours -->
            <div id="tabtimer-hourly-group" style="display:none; margin-top:8px;">
              <label style="font-size:12px;">Repeat every <input type="number" id="tabtimer-hourly-interval" min="1" max="23" value="6" style="width:55px;display:inline;padding:4px;border:1px solid #e2e8f0;border-radius:4px;"> hours</label>
            </div>
            <!-- Custom Days -->
            <div id="tabtimer-customdays-group" style="display:none; margin-top:8px;">
              <label style="font-size:12px;">Repeat every <input type="number" id="tabtimer-custom-days" min="2" max="365" value="3" style="width:55px;display:inline;padding:4px;border:1px solid #e2e8f0;border-radius:4px;"> days</label>
            </div>
            <!-- Specific Dates -->
            <div id="tabtimer-specificdates-group" style="display:none; margin-top:8px;">
              <label style="font-size:12px;">Specific dates (comma separated YYYY-MM-DD):</label>
              <input type="text" id="tabtimer-specific-dates" placeholder="2026-01-20, 2026-01-25" style="width:100%;margin-top:4px;padding:6px;border:1px solid #e2e8f0;border-radius:4px;box-sizing:border-box;">
            </div>
          </div>
          <input type="hidden" id="tabtimer-repeat-value" value="daily">
          <input type="hidden" id="tabtimer-minute-interval-value" value="10">
          <input type="hidden" id="tabtimer-hourly-interval-value" value="6">
          <input type="hidden" id="tabtimer-custom-days-value" value="3">
          <input type="hidden" id="tabtimer-specific-dates-value" value="">
        </div>
...
${[0,1,2,5,10,15,30].map(v => `<button type="button" class="tabtimer-lock-btn${defaultLock===v?' active':''}" data-value="${v}">${v===0?'Never':v+' min'}</button>`).join('')}
          </div>
          <div style="font-size: 11px; color: #64748b; margin-top: 6px;">
            💡 If you manually unlock a page, it will auto re-lock after this time
          </div>
          <input type="hidden" id="tabtimer-lock-minutes" value="${defaultLock}">
        </div>
        
        <div class="tabtimer-field">
          <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
            <input type="checkbox" id="tabtimer-auto-close" style="width: 18px; height: 18px;">
            ⏱️ Auto-close tab after opening
          </label>
          <div id="tabtimer-auto-close-group" style="display:none; margin-top:8px;">
            <label style="font-size:12px;">Close after <input type="number" id="tabtimer-auto-close-minutes" min="1" max="60" value="2" style="width:50px;display:inline;padding:4px;border:1px solid #e2e8f0;border-radius:4px;"> minutes</label>
          </div>
        </div>
...
function setupDialogEvents(overlay, url, isDark, existingLocks, settings) {
  ...
  overlay.querySelectorAll('#tabtimer-repeat .tabtimer-btn-option').forEach(btn => {
    btn.addEventListener('click', () => {
      // Block PRO-locked buttons for free users
      if (btn.dataset.pro === 'true') {
        // Flash a tooltip-style message
        let proMsg = overlay.querySelector('#tabtimer-pro-msg');
        if (!proMsg) {
          proMsg = document.createElement('div');
          proMsg.id = 'tabtimer-pro-msg';
          proMsg.style.cssText = 'margin-top:6px;padding:6px 10px;background:#fef3c7;border:1px solid #f59e0b;border-radius:6px;font-size:12px;color:#92400e;';
          proMsg.innerHTML = '⭐ This repeat option requires <strong>Premium</strong>. Start your free 7-day trial in the <a href="#" id="tabtimer-open-mgmt" style="color:#d97706;font-weight:bold;">TabTimer Management page</a>.';
          otherOptionsDiv.parentNode.insertBefore(proMsg, otherOptionsDiv);
        }
        proMsg.style.display = 'block';
        overlay.querySelector('#tabtimer-open-mgmt') && overlay.querySelector('#tabtimer-open-mgmt').addEventListener('click', (e) => {
          e.preventDefault();
          chrome.runtime.openOptionsPage();
        });
        return;
      }
      // Hide any pro message
      const proMsg = overlay.querySelector('#tabtimer-pro-msg');
  ...
  // Helper to show/hide sub-fields based on Other dropdown selection
  function updateOtherSubFields() {
    const val = otherSelect.value;
    overlay.querySelector('#tabtimer-minutes-group').style.display    = val === 'minutes'        ? 'block' : 'none';
    overlay.querySelector('#tabtimer-hourly-group').style.display     = val === 'hourly'         ? 'block' : 'none';
    overlay.querySelector('#tabtimer-customdays-group').style.display = val === 'custom-days'    ? 'block' : 'none';
    overlay.querySelector('#tabtimer-specificdates-group').style.display = val === 'specific-dates' ? 'block' : 'none';
    overlay.querySelector('#tabtimer-repeat-value').value = val;
  }

  // When Other dropdown changes, update hidden value and show sub-fields
  if (otherSelect) {
    otherSelect.addEventListener('change', updateOtherSubFields);
    // Sync live input values to hidden fields
    const minuteInput = overlay.querySelector('#tabtimer-minute-interval');
    const hourlyInput = overlay.querySelector('#tabtimer-hourly-interval');
    const customDaysInput = overlay.querySelector('#tabtimer-custom-days');
    const specificDatesInput = overlay.querySelector('#tabtimer-specific-dates');
    if (minuteInput) minuteInput.addEventListener('input', () => { overlay.querySelector('#tabtimer-minute-interval-value').value = minuteInput.value; });
    if (hourlyInput) hourlyInput.addEventListener('input', () => { overlay.querySelector('#tabtimer-hourly-interval-value').value = hourlyInput.value; });
    if (customDaysInput) customDaysInput.addEventListener('input', () => { overlay.querySelector('#tabtimer-custom-days-value').value = customDaysInput.value; });
    if (specificDatesInput) specificDatesInput.addEventListener('input', () => { overlay.querySelector('#tabtimer-specific-dates-value').value = specificDatesInput.value; });
  }
  
  // Lock duration buttons
  // Auto-close checkbox toggle
  const autoCloseCheckbox = overlay.querySelector('#tabtimer-auto-close');
  const autoCloseGroup = overlay.querySelector('#tabtimer-auto-close-group');
  if (autoCloseCheckbox && autoCloseGroup) {
    autoCloseCheckbox.addEventListener('change', () => {
      autoCloseGroup.style.display = autoCloseCheckbox.checked ? 'block' : 'none';
    });
  }
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    const notesEl = overlay.querySelector('#tabtimer-notes');
    const notes = notesEl ? notesEl.value.trim() : '';
    const autoCloseEl = overlay.querySelector('#tabtimer-auto-close');
    ...
    const minuteIntervalEl = overlay.querySelector('#tabtimer-minute-interval');
    const hourlyIntervalEl = overlay.querySelector('#tabtimer-hourly-interval');
    const customDaysEl = overlay.querySelector('#tabtimer-custom-days');
    const specificDatesEl = overlay.querySelector('#tabtimer-specific-dates');
    const minuteIntervalVal = minuteIntervalEl ? (parseInt(minuteIntervalEl.value) || 10) : 10;
    const hourlyIntervalVal = hourlyIntervalEl ? (parseInt(hourlyIntervalEl.value) || 6) : 6;
    const customDaysVal = customDaysEl ? (parseInt(customDaysEl.value) || 3) : 3;
    const specificDatesVal = specificDatesEl ? (specificDatesEl.value || '') : '';
    ...
    const result = await chrome.runtime.sendMessage({
      ...
      lock: {
        ...
        minuteInterval: minuteIntervalVal,
        hourlyInterval: hourlyIntervalVal,
        customDays: customDaysVal,
        specificDates: specificDatesVal,
